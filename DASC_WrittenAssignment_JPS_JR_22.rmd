...
- The first part, which contains various data tidying and visualizations by both Justus and Jan </br>
...
This part is written by Justus Reiffs. In this part will be performed a multivariate regression. Therefore the Educational attainment for 
the ages between 15 and 64 in secondary education for both genders are going to be compared to 
the percentage of being at risk of being in poverty by the NUTS regions.

### Collecting data 

First the libraries are needed. Therefore the ggplot2 and tidyverse libary is loaded. This gives the
possibility to use multiple commands and steps managing the data. 
...
"lm()" command. Inside that command the information is defined. Once we have the risk of poverty and 
the educational attainment. Also the information for the year and the dataset are included.
...
the min and max values are very close and only habe a difference in a view points.
...
The low F-statistic and p-value indicates that the variables having an significant relationship 
as well.
...
horizontal line for zero is to suggest that the model is corectly capturing the relationship 
of those variabels. Second the constant variance is important. It is to see that there is an constant spread of residuals.
That is called homoscedasticity and indicates an constant variance of the residants across the values. 

```{r}
plot(model, which = 2)
```

At first to highlight the collinerity if the diagram. The points on the diagram are nearly on a straight 
line which indicates that the data used is aproximatly normal.
The symmetry of the points also is very important. Here we can se that the points are roughly symmetrically
around the line. This indicates that the data is skewed.
Outliers are visible at the top right and are clearly apart from the other points.

```{r}
plot(model, which = 3)
```

The diagram shows a wide spreaded field of the points this indicates no clear pattern. This indicates 
that the model correctly captures the relationship in the data.
The constant variance is visible by its wide spread that indicates homoscedasticity and a constant 
