...
The following text was, if not stated otherwise, written by Jan Pinto Strohhäusl. This chapter will discuss different educational values of countries.
...
We only need data that relates to a country, not a region. However, we still have rows that are regions of a country. To filter these out, we enter the following command:

```{r}
final1 <- filtered1 %>% filter_all(all_vars(!is.na(.)))
```

Conveniantly, this also deletes all countries from the dataframe that have no data for that specific context. For the last step, we need to create a new row in our dataframe. This way, we can later in the diagram generate a legend.
...
First, we will create a linear regression model of the dataframe by using the `lm()` command. After that, we generate the summary of that model:
