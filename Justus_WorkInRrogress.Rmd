...
In this part will be performed a multyvariate regression. 

### collecting data 

first the libraries are needet 

```{r Unemployment rate - risk to poverty}
library(ggplot2)
library(tidyverse)

info <- read.csv("qog_eureg_wide1_nov20.csv")

#info <- filter(info, region_name == "GERMANY")

model <- lm(eu_povrisk_pc_nuts0 ~ eu_unemp_ge15t_nuts0 + year, data = info)
summary(model)
...
plot(eu_povrisk_pc_nuts0,eu_unemp_ge15t_nuts0)
```